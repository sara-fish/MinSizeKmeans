...
class subproblem(object):
    ...
    def create_model(self):
        ...
        self.model += pulp.lpSum(
            [distances(assignment) * self.y[assignment] for assignment in assignments]
        )
        ...
        for j in range(self.k):
            self.model += (
                pulp.lpSum(self.y[(i, j)] for i in range(self.n)) - self.min_size
                == self.b[j]
            )
        ...
        self.model += pulp.lpSum(self.b[j] for j in range(self.k)) == self.n - (
            self.k * self.min_size
        )

    def solve(self):
        self.status = pulp.GUROBI(msg=0).solve(self.model)
...
def initialize_centers(dataset, k, random_obj):
    ids = list(range(len(dataset)))
    random_obj.shuffle(ids)
...
def minsize_kmeans(dataset, k, min_size=0, max_size=None, random_obj=random):
    n = len(dataset)
    if max_size == None:
        max_size = n

    centers = initialize_centers(dataset, k, random_obj)
    clusters = [-1] * n
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--OUTFILE", help="store the result in OUTFILE", default=""
    )
    ...
    for i in range(args.NUM_ITER):
        clusters, centers = minsize_kmeans(data, args.k, args.min_size, args.max_size)
        if clusters:
